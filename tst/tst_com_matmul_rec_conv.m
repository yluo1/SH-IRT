function tst_com_matmul_rec_conv
%Test commutative properties of matrix multiplication and recursive convolution
...
for n = 1:N
    D_pre(n, :) = ft_rec_conv_opt(BC(n,:), g);
end

D_pst = zeros(N, M * numel(g)); %Post-matrix-mult
for n = 1:N
    D_pst(n, :) = ft_rec_conv_opt(C(n,:), g);
end
D_pst = B * D_pst;
...
D_pre = zeros(N, M * numel(g)); %Pre-matrix-mult
for n = 1:N
    D_pre(n, :) = ft_rec_conv_opt(B2C(n,:), g);
end
