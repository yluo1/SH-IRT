...
for l = 0:P
    ...
    if l == 0
        % y = y + sqrt((2*l + 1) / (4*pi)) ...
        %     * ( (1 - legendreP2(l+1, cos_theta) ) - (1 - legendreP2(l+1, ones(N, 1)) )  ) / (2*l + 1) ...
        %     * real(C_pdf(idx, :));  %Requires normalization

        % y = y + sqrt((2*l + 1) / (4*pi) ) ...
        %     * ( 1 - legendreP2(l+1, cos_theta)  ) / (2*l + 1) ...
        %     * real(C_pdf(idx, :)); %Requires normalization

        y = y + sqrt(pi / (2*l + 1)) ...
            * ( 1 - legendreP2(l+1, cos_theta)  ) ...
            * real(C_pdf(idx, :)); %No normalization

    else
        % y = y + sqrt((2*l + 1) / (4*pi)) ...
        %     * ( (legendreP2(l-1, cos_theta) - legendreP2(l+1, cos_theta) ) - (legendreP2(l-1, ones(N, 1)) - legendreP2(l+1, ones(N, 1)) )  ) / (2*l + 1) ...
        %     * real(C_pdf(idx, :));  %Requires normalization

        % y = y + sqrt((2*l + 1) / (4*pi)) ...
        %     * ( (legendreP2(l-1, cos_theta) - legendreP2(l+1, cos_theta) ) ) / (2*l + 1) ...
        %     * real(C_pdf(idx, :)); %Requires normalization

        y = y + sqrt(pi / (2*l + 1)) ...
            * ( (legendreP2(l-1, cos_theta) - legendreP2(l+1, cos_theta) ) ) ...
            * real(C_pdf(idx, :)); %No normalization
    end
end
%y = y * 2 * pi; %Normalize
...
if nargout > 1 % Compute derivative of CDF
    ...
    for l = 0:P
        idx = (l + 1)^2 - l;
%        dy = dy + sqrt((2*l + 1) / (4*pi)) * legendreP2(l, cos_theta) *  real(C_pdf(idx, :)); %Requires normalization
        dy = dy + sqrt((2*l + 1) * pi) * legendreP2(l, cos_theta) *  real(C_pdf(idx, :)); %No normalization
