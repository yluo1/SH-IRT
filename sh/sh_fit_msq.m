...
%Least-squares magnitude squared SH expansion to observations
%Y(theta, phi) * D = abs(Y(theta, phi) * C)^2 \approx X(theta, phi)
...
%C:                 [(floor(max_odr/2) + 1)^2 x M] SH coefficients of dominant polynomial
...
%is_real = true;
%D_ref = sh_msq(sh_rand(max_odr_half, 1, is_real), is_real);
%[theta, phi] = sh_fib(N);
%X = sh_dec(D_ref, theta, phi, is_real);
%C0 = randn((floor(max_odr/2) + 1)^2, 50);
%[D, C, err] = sh_fit_msq(X, theta, phi, max_odr, is_real, 'fmincon', 'C0', C0);
%err
%sh_plt(D_ref, 'mercator', is_real, 'title_name', 'Ref. D');
%sh_plt(D, 'mercator', is_real, 'title_name', 'fit D');
%sh_plt(C, 'mercator', is_real, 'title_name', 'fit C');
...
arguments
    ...
    mode (1,:) char {mustBeMember(mode, {'fmincon', 'sdp'})} = 'fmincon';
...
elseif strcmp(mode, 'sdp')
    ...
    for m = 1:M
        ...
        D_m = sum(sh_msq(C_eig_vec(:, idx_eigs) * diag(sqrt(C_eig_val(idx_eigs))), true), 2); %Real
        D(1:numel(D_m), m) = D_m;
    end
