...
%Format \prod_n H(SOS_n):
%varargin{n} = SOS  [N_SOS x 6] Second-order sections

%Format \prod_n H(b_n) / H(a_n):
%varargin{2 * n - 1}    = b    [N_b x 1] or [1 x N_b] numerator coefficients
%varargin{2 * n}        = a    [N_a x 1] or [1 x N_a] denominator coefficients

%Format \prod_n H(SOS_n) * H(b_n) / H(a_n):
%varargin{3 * n - 2}  = b    [N_b x 1] or [1 x N_b] numerator coefficients
%varargin{3 * n - 1}  = a    [N_a x 1] or [1 x N_a] denominator coefficients
%varargin{3 * n}      = SOS  [N_SOS x 6] Second-order sections
