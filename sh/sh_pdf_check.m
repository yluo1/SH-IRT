function is_valid_pdf = sh_pdf_check(C_pdf, is_real)
...
if is_real
    ...
    is_real_field = all(vecnorm(C_pdf_cpx - sh_conj(C_pdf_cpx)) <= 1e-8);
else
    is_real_field = all(vecnorm(C_pdf - sh_conj(C_pdf)) <= 1e-8);    
end
%Note: non-negative requires sum-of-square factorization of expansion

%Check if field integrates to unity
is_unity_integral = all(abs(sh_int(C_pdf, 'Sum') - 1) <= 1e-8);
