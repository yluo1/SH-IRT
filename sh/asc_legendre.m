function P = asc_legendre(n, x, normalize)
...
elseif strcmp(normalize,'norm')
    ...
    error('InvalidNormalize');
end

% Restore original dimensions.
if length(sizex) > 2 || min(sizex) > 1
    P = reshape(y,[n+1 sizex]);
else
    P = y;
end

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function x = multiply(vec, x)
% carefully computing prod(vec)*x
scale = prod(vec); 
if isinf(scale)
    for i = 1:numel(x)
      x(i) = prod([x(i) vec]);
